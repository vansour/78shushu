...
impl QuestionGenerator {
    ...
    fn format_question_response(question: Question) -> QuestionResponse {
        let options_labeled = question.options
            .iter()
            .enumerate()
            .map(|(i, option)| {
                let label = match i {
    ...
    fn get_explanation(question: &Question) -> Option<String> {
        match question.id {
            1 => Some("AK-74是经典的苏式突击步枪，标准弹匣容量为30发，这是大多数突击步枪的标准配置。".to_string()),
            2 => Some("AWP是著名的狙击步枪，具有极高的伤害和射程，适合远距离精确射击。".to_string()),
            3 => Some("Vector冲锋枪以其超高的射速而闻名，在近距离战斗中具有压倒性优势。".to_string()),
            4 => Some("狙击手需要占据制高点来获得更好的视野和射击角度，瞭望台是理想的狙击位置。".to_string()),
            7 => Some("医疗兵是团队的生命保障，应该优先救治受伤队友，保持团队战斗力。".to_string()),
            8 => Some("烟雾弹可以有效阻挡狙击手视线，为团队提供移动掩护。".to_string()),
            12 => Some("支援兵负责为团队提供持续的火力支援和弹药补给，是团队的火力核心。".to_string()),
            _ => None,
        }
    }
}